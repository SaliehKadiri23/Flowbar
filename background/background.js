...
async function calculateFlowScore(domain, timeData) {
  try {
    ...
    const distractionSites = (settings.distractionSites || "")
      .split(",")
      .map((site) => site.trim())
      .filter((site) => site);
    const focusSites = (settings.focusSites || "")
      .split(",")
      .map((site) => site.trim())
      .filter((site) => site);

    // Get all sessions for the specific domain
    const domainSessions = timeData.sessionHistory.filter(
    ...
    const domainFocusTime = domainSessions
      .filter(
        (session) =>
          session.type === "focus" &&
          !matchesDomain(session.domain, distractionSites)
      )
    ...
    if (matchesDomain(domain, distractionSites)) {
      ...
      const focusPercentage =
        domainFocusTime > 0 ? (domainFocusTime / totalTime) * 100 : 0;
        ...
        try {
          await chrome.scripting.executeScript({
            ...
            func: (borderColor) => {
              ...
              const existingBorders = document.querySelectorAll(
                '[class*="flowbar-border"]'
              );
          ...
          console.warn(
            "Flowbar background.js warning: Could not inject into tab:",
            tab.url,
            injectError.message
          );
        }
...
async function getTimerState() {
  try {
    ...
    if (
      result.endTime &&
      (result.timerState === "focus" || result.timerState === "break")
    ) {
      const now = Date.now();
    ...
    return {
      ...
      timeLeft: timeLeft,
      totalDuration: result.totalDuration || focusDuration, // Use focusDuration as default for initial state
    ...
    return {
      ...
      totalDuration: 25 * 60, // Default to 25 minutes
      focusDuration: 25 * 60,
      breakDuration: 5 * 60,
    };
  }
 ...
 * @param {Object} state - The state object containing timerState, originalTimerType, endTime, timeLeft, and totalDuration
 ...
 * @param {number} [state.totalDuration] - Total duration of the timer in seconds (optional)
 * @returns {Promise<void>}
 */
async function setTimerState(state) {
  try {
    ...
    // Only include totalDuration if it's provided
    if (state.totalDuration !== undefined) {
      storageObj.totalDuration = state.totalDuration;
    }
...
async function startTimer(durationInMinutes, state) {
  try {
    ...
    // Set the timer state with totalDuration to allow progress ring to calculate percentages correctly
    const totalDuration = durationInMinutes * 60; // Convert minutes to seconds for total duration
    await setTimerState({
      timerState: state,
      endTime: endTime,
      timeLeft: timeLeft,
...
async function stopTimer() {
  try {
    ...
    await setTimerState({
      ...
      totalDuration: focusDuration, // Reset to initial focus duration as total
    });
        ...
        if (state.endTime) {
          ...
          if (now - lastTimerStateUpdate >= TIMER_STATE_UPDATE_INTERVAL) {
            ...
            await setTimerState({
              ...
              endTime: state.endTime,
              timeLeft: remainingSecs,
              totalDuration: state.totalDuration, // Maintain the total duration for progress calculation
            });
          }
    ...
    if (request.action === "startTimer") {
      ...
      if (currentState.timerState === "stopped") {
        ...
        return Promise.resolve({
          success: false,
          error: "Use resumeTimer action to resume a paused timer",
        });
      } else {
        ...
        if (
          ...
          currentState.timerState === "wind-down"
        ) {
          const now = Date.now(); // Define 'now' variable
          const timeData = await getTimeData();
          const ongoingSessionIndex = timeData.sessionHistory.findIndex(
            (session) =>
      ...
      if (
        ...
        await setTimerState({
          ...
          endTime: currentState.endTime, // Keep the original endTime for potential resume
          timeLeft: currentRemainingTime, // Preserve the exact remaining time
          totalDuration: currentState.totalDuration, // Maintain the total duration for progress calculation
        });
        await chrome.alarms.clearAll(); // Clear active alarms when paused
      ...
      if (currentActiveTabId && currentDomain) {
        const now = Date.now();
        if (now - lastIconUpdateTime > ICON_UPDATE_DEBOUNCE_MS) {
          ...
          try {
            ...
            const domainSessions = timeData.sessionHistory.filter(
              (session) => session.domain === currentDomain
            );
            ...
            const domainFocusTime = domainSessions
              .filter(
                (session) =>
                  session.type === "focus" &&
                  !matchesDomain(session.domain, distractionSites) &&
                  !matchesDomain(session.domain, focusSites)
              )
              .reduce((total, session) => total + session.duration, 0);
            ...
            const distractionMinutes = Math.floor(
              (domainDistractionTime % 3600) / 60
            );

            // Determine if current domain is a distraction site
            const isCurrentDistraction = matchesDomain(
              currentDomain,
              distractionSites
            );
            ...
            const modalContent = `
              ...
              <div class="flowbar-modal-content">
                ...
                ${
                  isCurrentDistraction
                    ? '<p class="distraction-warning"><strong>This is a distraction site</strong></p>'
                    : ""
                }
              </div>
            `;
            ...
            await chrome.scripting.executeScript({
              ...
              func: (score, content) => {
                ...
                if (
                  ...
                  window.flowbarTitleModifier.prependIconToTitle
                ) {
                ...
                if (
                  window.flowbarTitleModifier &&
                  window.flowbarTitleModifier.createHoverModal
                ) {
            ...
            console.warn(
              "Flowbar background.js: Could not update icon on active tab after pause:",
              scriptError.message
            );
          }
        }
          ...
          try {
            ...
            const modalContent = `
              ...
              <div class="flowbar-modal-content">
                ...
                <p><strong>Focus Time:</strong> ${focusHours}h ${focusMinutes}m</p>
                <p><strong>Distraction Time:</strong> ${distractionHours}h ${distractionMinutes}m</p>
                <p style="margin-bottom: 20px"><strong>Sessions:</strong> ${
            ...
            console.warn(
              "Flowbar background.js: Could not update icon on active tab after resume:",
              scriptError.message
            );
          }
        }
      ...
      if (currentActiveTabId && currentDomain) {
        ...
        if (now - lastIconUpdateTime > ICON_UPDATE_DEBOUNCE_MS) {
          ...
          try {
            ...
            console.warn(
              "Flowbar background.js: Could not update icon on active tab after reset:",
              scriptError.message
            );
          }
        }
      ...
      if (currentActiveTabId && currentDomain) {
        ...
        if (now - lastIconUpdateTime > ICON_UPDATE_DEBOUNCE_MS) {
          ...
          try {
            ...
            console.warn(
              "Flowbar background.js: Could not update icon on active tab after stop:",
              scriptError.message
            );
          }
        }
  ...
  try {
    ...
    if (tab && tab.url) {
      ...
      try {
        ...
        console.error(
          "Flowbar background.js error calculating flow score:",
          error
        );
      }
      ...
      const domainFocusTime = domainSessions
        .filter(
          (session) =>
            session.type === "focus" &&
            !distractionSites.includes(session.domain) &&
            !focusSites.includes(session.domain)
        )
        .reduce((total, session) => total + session.duration, 0);
      ...
      try {
        ...
        console.warn(
          "Flowbar background.js: Could not execute title-modifier in tab:",
          scriptError.message
        );
      }
    }
...
async function initializeActiveTab() {
  try {
    ...
    console.error(
      "Flowbar background.js error initializing active tab:",
      error
    );
  }
}
...
async function checkDistractionSite(tabId, url) {
  try {
    ...
    if (
      ...
      matchesDomain(currentDomain, distractionSites)
    ) {
      // Check if this domain has been granted temporary access (for 60 seconds)
      const tempAccess = await chrome.storage.local.get([
        `tempAccess_${currentDomain}`,
      ]);
      const now = Date.now();

      if (
        !tempAccess[`tempAccess_${currentDomain}`] ||
        now > tempAccess[`tempAccess_${currentDomain}`]
      ) {
        // Redirect to sanctuary page
        const sanctuaryUrl = chrome.runtime.getURL(
          "sanctuary.html?site=" +
    ...
    console.error(
      "Flowbar background.js error checking distraction site:",
      error
    );
    return false;
  }
  ...
  trackingInterval = setInterval(async () => {
    try {
      ...
      if (timerState.timerState === "focus" && currentDomain) {
        ...
        const isDistractionSite = matchesDomain(
          currentDomain,
          distractionSites
        );
        ...
        const activeSessionIndex = timeData.sessionHistory.findIndex(
          (session) =>
            session.domain === currentDomain &&
            session.type === "focus" &&
            !session.endTime // Incomplete session
      ...
      if (currentActiveTabId && currentDomain) {
        ...
        if (now - lastIconUpdateTime > ICON_UPDATE_DEBOUNCE_MS) {
          ...
          try {
            ...
            console.warn(
              "Flowbar background.js: Could not update icon on active tab after state change:",
              scriptError.message
            );
          }
        }
...
chrome.runtime.onInstalled.addListener(async (details) => {
  ...
  if (details.reason === "install") {
    ...
    try {
      ...
      console.error(
        "Flowbar background.js error opening options page on install:",
        error
      );
      ...
      try {
        ...
        console.error(
          "Flowbar background.js error with fallback options page:",
          fallbackError
        );
      }
    }
  ...
  try {
    ...
    if (
      ...
      await setTimerState({
        ...
        totalDuration: currentState.totalDuration, // Preserve total duration
      });
      // Complete any ongoing session for the current domain
      const timeData = await getTimeData();
      const ongoingSessionIndex = timeData.sessionHistory.findIndex(
